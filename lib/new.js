...
function New (options) {
  ...
  if (options.mixing && options.mixing.length) {
    brick.mixing = mix(brick, options.mixing, options.dom && options.dom[MIXINGS_KEY]);
  }
...
function mix (brick, mixings, dom) {
  ...
  brick.mixings = mixings.map(function (Factory) {
    var b = Factory.New({ brick: { dom: dom && dom[Factory._brick.meta.key] } }).brick;
